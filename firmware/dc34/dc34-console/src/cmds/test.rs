...
impl<'a> ShellCmdApi<'a> for Test {
    ...
    fn process(&mut self, args: String, _env: &mut CommonEnv) -> Result<Option<String>, xous::Error> {
        ...
        match cmd.as_str() {
            ...
            // ungated: full-spectrum rainbow.
            //   `test rainbow`        -> persistent, fast (survives reboot)
            //   `test rainbow <0-15>` -> live-tune the hue cycling rate (15 = fastest), transient
            "rainbow" => {
                let conn = _env.xns.request_connection_blocking(dc34_api::LED_SERVER).unwrap();
                // full spectrum (hue 0..255), max saturation, no chaser, snappy frames.
                // low cd_rate -> short frame period (tau); hue_ratedir low-nibble = hue cycling speed.
                let base = Haploid {
                    cd_period: 4,
                    cd_rate: 25,
                    cd_dir: 200,
                ...
                if let Some(sv) = args.get(0).and_then(|s| u8::from_str_radix(s, 10).ok()) {
                    // live tune: Force the exact hue rate (bypasses the gene-blend clamp), transient
                    let sv = sv.min(15);
                    let pheno = Haploid { hue_ratedir: sv, ..base };
                    let a = pheno.serialize_u32();
                    xous::send_message(
                        conn,
                        xous::Message::new_blocking_scalar(
                            dc34_api::LedManagerOp::Force.to_usize().unwrap(),
                    ...
                    .ok();
                    write!(ret, "rainbow forced @ speed {}", sv).ok();
                } else {
                    // persistent: gene blend maps hue_ratedir sum (2+1=3) -> phenotype 13 (near-max rate)
                    let g0 = Haploid { hue_ratedir: 2, ..base };
                    let g1 = Haploid { hue_ratedir: 1, ..base };
                    let gene = Diploid([g0, g1]);
                    save_light_gene(gene);
                    gene.send(conn, LedManagerOp::SetGene.to_usize().unwrap());
                    write!(ret, "rainbow set (persistent, fast)").ok();
                }
            }
            // ungated: snap one camera frame and dump it over the serial log (badge "polaroid").
            // the video service does the work; a host script reads the PHOTOSTART/PHOTO/PHOTOEND lines.
            "photo" => {
                let gfx = ux_api::service::gfx::Gfx::new(&_env.xns).unwrap();
                // arm the one-frame dump in the video service
                xous::send_message(
                    gfx.conn(),
                    xous::Message::new_scalar(
                        ux_api::service::api::GfxOpcode::CapturePhoto.to_usize().unwrap(),
                        0,
                        0,
                        0,
                        0,
                ...
                .ok();
                // start the camera; the video service dumps the frame, then ends the acquisition
                let _ = gfx.acquire_qr();
                write!(ret, "photo dumped over serial").ok();
            }
            "jig" => {
