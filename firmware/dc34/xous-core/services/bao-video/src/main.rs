...
pub fn wrapped_main(main_thread_token: MainThreadToken) -> ! {
    ...
    let mut photo_req = false; // CapturePhoto sets this: dump one settled frame over the serial log
    ...
    loop {
        if !is_panic.load(Ordering::Relaxed) {
            ...
            match opcode {
                #[cfg(not(feature = "hosted-baosec"))]
                GfxOpcode::CapturePhoto => {
                    // arm a one-frame dump; the camera is started by the AcquireQr that follows
                    photo_req = true;
                }
                #[cfg(not(feature = "hosted-baosec"))]
                GfxOpcode::AcquireQr => {
                    if qr_request.is_none() {
                        ...
                        while retries < RETRY_LIMIT {
                            ...
                            // NOTE: preemption is DISABLED here (set_preemption(false) above), so
                            // std::time::Instant::now() -- which calls the ticktimer *server* -- would
                            // block forever waiting on a process that can't be scheduled, freezing the
                            // whole system at "Starting camera...". Use a bounded busy-spin counter so
                            // the startup wait can never depend on a service that can't run. If the
                            // camera doesn't raise its IRQ within the bound, we fall through to the
                            // existing retry/reset (and eventual reboot) path instead of hanging.
                            const STARTUP_SPINS: u32 = 200_000_000;
                            ...
                            let mut spins: u32 = 0;
                            while spins < STARTUP_SPINS {
                                // this effectively halts anything else from happening until a CamIrq is
                                // produced -- but is now bounded so it cannot hang indefinitely
                ...
                GfxOpcode::KeyPress => {
                    if let Some(scalar) = msg.body.scalar_message() {
                        ...
                        if !(k == '🔽' || k == '🔼') {
                            // any key press ends the camera: in photo mode it SNAPS (dumps the
                            // current viewfinder frame over serial); otherwise it just aborts the scan.
                            if let Some(mut envelope) = qr_request.take() {
                                #[cfg(not(feature = "hosted-baosec"))]
                                {
                                    hal.set_preemption(true); // must be on before logging
                                    if photo_req {
                                        photo_req = false;
