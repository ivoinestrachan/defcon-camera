...
impl GlobalConfig {
    ...
    pub fn pause_led_motion(&self, pause: bool) {
        // tell the console LED/motion side to stop polling I2C during a camera scan, so it
        // doesn't contend on the shared I2C bus and wedge the camera bring-up
        xous::send_message(
            self.led_server,
            xous::Message::new_scalar(
                LedManagerOp::Pause.to_usize().unwrap(),
                if pause { 1 } else { 0 },
                0,
                0,
                0,
