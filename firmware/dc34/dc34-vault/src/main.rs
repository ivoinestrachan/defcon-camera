                ...
                if menu_active {
                    ...
                    match k.unwrap_or('\0') {
                        ...
                        '🔥' => {
                            ...
                            // also pause the console motion thread's I2C polling so it doesn't
                            // contend on the shared bus and wedge the camera bring-up
                            global_config.lock().unwrap().pause_led_motion(true);
                            // take a PHOTO (not a QR scan): arm the one-frame dump so the camera run
                            // snaps a frame and dumps it over serial instead of decoding a code
                            xous::send_message(
                                gfx.conn(),
                            ...
                            global_config.lock().unwrap().pause_led_motion(false);
